...
for tracer in ['LRG', 'ELG_LOP', 'ELG_VLO', 'QSO', 'BGS_BRIGHT', 'BGS_FAINT']:
    ...
    if stats_combined is None:
        stats_combined = stats.copy()
    else:
        stats_combined = join(stats_combined, stats, keys='FIBER', join_type='outer').filled(-99)

    print()

stats_combined.sort('FIBER')
stats_combined.write(output_fn, overwrite=True)
