...
mask = (cat['OII_FLUX']>0) & (cat['OII_FLUX_IVAR']>0)
mask &= np.log10(cat['OII_FLUX'] * np.sqrt(cat['OII_FLUX_IVAR'])) > 0.9 - 0.2 * np.log10(cat['DELTACHI2'])
print('OII cut  ', np.sum(~mask), np.sum(mask), np.sum(~mask)/len(mask))
cat = cat[mask]
...
fiberstats.write('/global/cfs/cdirs/desicollab/users/rongpu/redshift_qa/jura_data/ks_fiberstats_{}_lss.fits'.format(tracer.lower()), overwrite=True)

fiberstats = Table(fitsio.read('/global/cfs/cdirs/desicollab/users/rongpu/redshift_qa/jura_data/ks_fiberstats_{}_lss.fits'.format(tracer.lower())))
...
with PdfPages('/global/cfs/cdirs/desicollab/users/rongpu/redshift_qa/per_fiber_qa/jura/per_fiber_redshifts_{}_lss_catalog.pdf'.format(tracer.lower())) as pdf:
    # for ii in range(1):
